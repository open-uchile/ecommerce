  ...
  return Backbone.View.extend({
    ...
    changePage: function () {
      this.$el.html(
        this.template({
          ...
          pageHeading: this.$el
            .closest("#offerApp")
            .data("offerAppPageHeading"),
          pageHeadingMessage: this.$el
            .closest("#offerApp")
            .data("offerAppPageHeadingMessage"),
    ...
    showEmptyOfferErrorMessage: function () {
      this.$el.html(
        this.errorTemplate({
          error_msg:
            "This coupon is not valid for any currently available course seats.",
        })
      );
    },
    ...
    refreshData: function () {
      ...
      this.isEnrollmentCode =
        benefitData.type === "Percentage" &&
        Math.round(benefitData.value) === 100;
    ...
    formatDate: function (course) {
      // EOL Change text and locale
      moment.locale("es");
      var courseStartDateText = gettext(
          _s.sprintf(
            "Fecha de Inicio: %s",
            moment(course.get("course_start_date")).format("MMM DD, YYYY")
          )
        ),
        voucherEndDateText = gettext(
          _s.sprintf(
            "Descuento valido hasta %s",
    ...
    setNewPrice: function (course) {
      ...
      if (
        course.get("seat_type") === "credit" &&
        !course.multiple_credit_providers
      ) {
        price = parseFloat(course.get("credit_provider_price")).toFixed(2);
      } else {
        price = parseFloat(course.get("stockrecords").price_excl_tax).toFixed(
          2
        );
      }
    ...
    renderPagination: function () {
      ...
      this.pagination.append(
        this.createPreviousItem(this.collection.onFirstPage())
      );
      ...
      if (this.collection.page > 1) {
        this.pagination.append(
          this.createListItem(this.collection.page - 1, false)
        );
      }
      ...
      if (this.collection.page + 1 < numberOfPages) {
        if (this.collection.page === numberOfPages - showEllipsisBeforeRange) {
          ...
          numberOfPages - showEllipsisBeforeRange
        ) {
          this.pagination.append(this.createEllipsisItem());
        }
